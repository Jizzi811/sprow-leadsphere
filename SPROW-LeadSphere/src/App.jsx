...
export function App() {
  ...
  const runLive = async raw => {
    const targetUrl = /^https?:\/\//i.test(raw) ? raw : `https://${raw}`
    let host = raw; try { host = new URL(targetUrl).hostname.replace(/^www\./, '') } catch { /* noop */ }
    setLive(true); setLeads([]); setProgress(0); setNotice(`Live-Recherche läuft: ${host} …`)
    try {
      const res = await fetch(`${API_URL}/api/extract`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: targetUrl }) })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(data.detail || data.error || `Server antwortete mit ${res.status}`)
      const emails = data.emails || [], phones = data.phones || []
      setLeads([{ company: (data.title || host).slice(0, 90), city: phones[0] || '—', website: host, email: emails[0] || '—', score: Math.min(99, 45 + emails.length * 15 + phones.length * 10) }])
      setRuns(r => [{ query: host, region, target, count: 1, date: 'gerade eben' }, ...r].slice(0, 6))
      setNotice(emails.length || phones.length ? `Echte Kontaktdaten von ${host} extrahiert.` : `${host} geprüft – keine öffentlichen Kontaktdaten gefunden.`)
    } catch (err) {
      setNotice(`Live-Recherche fehlgeschlagen: ${err.message}`)
    } finally { setLive(false) }
  }
  const startSearch = e => {
    e?.preventDefault(); setActive('Neue Recherche')
    const q = query.trim()
    if (!q) { setNotice('Bitte beschreibe zuerst, wen LeadSphere finden soll.'); return }
    const urlMatch = q.match(/((?:https?:\/\/)?[a-z0-9-]+(?:\.[a-z0-9-]+)+(?:\/[^\s]*)?)/i)
    if (API_URL && urlMatch) { runLive(urlMatch[0]); return }
    setNotice(''); setLeads([]); setProgress(0); setSearching(true)
  }
  ...
  return <div className="app-shell">
    ...
    {active === 'Neue Recherche' && <><section className="hero"><div className="intro"><span className="eyebrow">SPROW LEADSPHERE</span><h1>Wen sollen wir<br/>heute finden?</h1><p>Dein Recherche-Agent für öffentlich erreichbare<br/>Geschäftskontakte im Energiemarkt.</p></div><ParticleOrb searching={searching || live}/></section>
    <form className="search-panel" onSubmit={startSearch}><div className="command"><Sparkle weight="fill"/><input value={query} onChange={e => setQuery(e.target.value)} aria-label="Rechercheauftrag"/><button type="button" onClick={() => setQuery('')} aria-label="Eingabe leeren"><X/></button></div><div className="filter-row"><label><MapPin/><select aria-label="Region" value={region} onChange={e => setRegion(e.target.value)}><option>Nordrhein-Westfalen</option><option>Deutschlandweit</option><option>Bayern</option><option>Hessen</option><option>Hamburg</option></select></label><label><Users/><select aria-label="Zielgruppe" value={target} onChange={e => setTarget(e.target.value)}><option>Hausverwaltungen</option><option>Wohnungsunternehmen</option><option>Kommunen</option><option>Gewerbebetriebe</option><option>Solar-Fachpartner</option></select></label><button className="start" disabled={searching || live}>{searching ? `Recherche ${progress}%` : live ? 'Recherche läuft …' : 'Suche starten'}<ArrowRight/></button></div>{searching && <div className="progress"><span style={{width:`${progress}%`}}/></div>}</form>
    {API_URL && <p className="live-hint">Live-Modus aktiv · Gib eine Firmen-Website ein (z.&nbsp;B. <b>example.de</b>), um echte Kontaktdaten zu extrahieren.</p>}
